(function(){
  ...
  if (statEl && 'IntersectionObserver' in window){
    ...
    // $3,675: high end of the local water heater replacement range. See THE MATH comment block.
    var STAT_TARGET = 3675;
    var countRun = 0;
  ...
  // -- Sticky mobile CTA bar: show at <=820px, hide while the real CTA panel is in view --
  var stickyCta = document.getElementById('stickyCta');
  var ctaPanel = document.querySelector('.cta-panel');
  var ctaVisible = false;
  var mobileQuery = window.matchMedia ? window.matchMedia('(max-width: 820px)') : { matches: false };

  function updateSticky(){
    if (!stickyCta) return;
    var shouldShow = mobileQuery.matches && !ctaVisible;
    stickyCta.classList.toggle('show', shouldShow);
    document.body.classList.toggle('has-sticky', shouldShow);
  }
