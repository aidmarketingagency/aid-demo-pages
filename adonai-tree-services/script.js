(function(){
  // prefers-reduced-motion gate (covers JS-driven animation per v2 spec)
  ...
  function playThread(){
    ...
    resetThread();
    var seq = [
      { t: 240,  action: function(){ bubbles[0].classList.add('show'); } },
      ...
      { t: 3280, action: function(){ typers[2].classList.add('show'); } },
  ...
  // Re-arm on scroll re-entry (v2 spec: nothing plays once and dies)
  ...
  // -- Sticky mobile CTA bar: hide while real CTA panel is in view (v2.1 A1) --
  ...
  // -- Animated stat counter, re-arms on scroll re-entry (v2 spec animation reset standard) --
  ...
  if (statEl){
    ...
    statEl.appendChild(plusSpan);

    // $3,000 — high end of Charlotte storm tree removal range, single tree
    // Source: https://www.angi.com/articles/how-much-does-tree-removal-cost.htm
    var STAT_TARGET = 3000;
    var countRun = 0;
