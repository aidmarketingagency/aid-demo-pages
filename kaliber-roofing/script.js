(function(){
  ...
  function playThread(){
    ...
    var seq = [
      ...
      { t: 4300, action: function(){ typers[2].classList.remove('show'); bubbles[3].classList.add('show'); } },
      { t: 5200, action: function(){ if (bubbles[4]) bubbles[4].classList.add('show'); playing = false; } }
    ];
  ...
  if (statEl && 'IntersectionObserver' in window){
    ...
    // $12,000: average insurance payout for a hail-damaged roof, re-verified 2026-07-16.
    // Sources in THE MATH comment block above.
  ...
  // -- A1 sticky mobile CTA bar: tuck it away while the real CTA panel is in view --
  ...
  if (stickyCta && ctaSection && 'IntersectionObserver' in window){
    var ctaIO = new IntersectionObserver(function(entries){
      entries.forEach(function(e){
        if (e.isIntersecting){ stickyCta.classList.add('tucked'); }
        else { stickyCta.classList.remove('tucked'); }
      });
    }, { threshold: 0.2 });
