(function(){
  ...
  // -- Sticky mobile CTA bar (v2.1 A1): hides itself while the real CTA panel is in view --
  var mobileCta = document.getElementById('mobileCta');
  var ctaPanelEl = document.querySelector('.cta-section');
  if (mobileCta){
    document.body.classList.add('has-mobile-cta');
    if (ctaPanelEl && 'IntersectionObserver' in window){
      var ctaIO = new IntersectionObserver(function(entries){
        entries.forEach(function(e){
          if (e.isIntersecting){ mobileCta.classList.add('is-hidden'); }
          else { mobileCta.classList.remove('is-hidden'); }
        });
      }, { threshold: 0.05 });
      ctaIO.observe(ctaPanelEl);
    }
  }
})();
