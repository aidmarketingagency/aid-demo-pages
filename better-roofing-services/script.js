(function(){
  ...
  // -- Sticky mobile CTA bar visibility (A1): hidden while the real CTA panel is
  //    in view, and while the site is not at mobile width, so only one CTA shows.
  var mobileCta = document.getElementById('mobileCta');
  var ctaPanelSection = document.querySelector('.cta-section');
  if (mobileCta && ctaPanelSection){
    function isMobileWidth(){ return window.matchMedia('(max-width: 820px)').matches; }

    function applyBodyPadding(){
      if (isMobileWidth() && !mobileCta.classList.contains('hide')){
