(function(){
  ...
  if (statEl && 'IntersectionObserver' in window){
    ...
    centsSpan.textContent = ',400';
    statEl.appendChild(dollarNode);
    statEl.appendChild(centsSpan);

    // $15,400: average homeowners insurance claim for water damage or freezing.
    // Source: https://www.consumeraffairs.com/homeowners/water-damage-insurance-claims-statistics.html (see THE MATH comment block)
    var STAT_TARGET = 15400;
    var countRun = 0;
  ...
  // -- Sticky mobile CTA bar (A1): hide while the real CTA panel is in view, one CTA posture --
  var stickyCta = document.getElementById('stickyCta');
  var ctaPanelSection = document.querySelector('[data-section="cta"]');
  if (stickyCta && ctaPanelSection && 'IntersectionObserver' in window){
    ...
    var heroIO = new IntersectionObserver(function(entries){
      entries.forEach(function(e){
        if (!e.isIntersecting && e.boundingClientRect.top < 0){ armed = true; }
        if (armed && window.matchMedia('(max-width:820px)').matches){ showSticky(); }
      });
    }, { threshold: 0 });
    ...
    var ctaIO = new IntersectionObserver(function(entries){
      entries.forEach(function(e){
        if (e.isIntersecting){ hideSticky(); }
        else if (armed && window.matchMedia('(max-width:820px)').matches){ showSticky(); }
      });
    }, { threshold: 0.1 });
    ctaIO.observe(ctaPanelSection);
  }
})();
